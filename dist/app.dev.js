...
var UIController = function () {
  ...
  var nodeListForEach = function nodeListForEach(list, callbackFn) {
  ...
  return {
    ...
    addListItem: function addListItem(object, type) {
      ...
      newHtml = newHtml.replace('%value%', formatNumber(object.value, type)); //insert the HTML to the DOM
    ...
    displayBudget: function displayBudget(obj) {
      ...
      document.querySelector(DOM_strings.incomeLabel).textContent = formatNumber(obj.totalInc, 'inc');
      document.querySelector(DOM_strings.expensesLabel).textContent = formatNumber(obj.totalExp, 'exp');
    ...
    displayMonth: function displayMonth() {
      ...
      month = now.getMonth();
      document.querySelector(DOM_strings.dateLabel).textContent = "".concat(months[month], "  ").concat(year);
    },
    changedType: function changedType() {
      var fields = document.querySelectorAll(DOM_strings.inputType + ',' + DOM_strings.inputDescription + ',' + DOM_strings.inputValue);
...
var controller = function (budgetCtrl, UICtrl) {
  ...
  var updatePercentages = function updatePercentages() {
    // 1. calculate the %.
    budgetCtrl.calculatePercentages(); //2. Read from the budget controller.

    var percentages = budgetCtrl.getPercentages(); //3. Update the UI.
  ...
  var ctrlAddItem = function ctrlAddItem() {
    ...
    if (input.description !== "" && !isNaN(input.value) && input.value > 0) {
      ...
      updateBudget(); //6. Calculate and update the %

      updatePercentages();
    }
  };

  var ctrlDeleteItem = function ctrlDeleteItem(event_click) {
    ...
    if (itemID) {
      ...
      updateBudget(); //4. Calculate and update the %
